...
class property extends CI_Controller{
	...
	function openForm(){
		...
		$this->load->view('postingform',$data);	
	}
   
	function image_upload() {

		$target_dir = "/var/www/html/100acres/public/images/posting/";
	...
	function submit(){ 
		...
		$property = array(
			'property_type' => $this->input->post('type2'),
			'city' => $this->input->post('type3'),
			'address' => $this->input->post('address'),
			'bedrooms' => $this->input->post('type5'),
			'expected_price' => $val4,
			'image_path_name' =>$name,
			'pp_area' =>$this->input->post('type10'),
			'description' => $this->input->post(trim(('description')))
		);		
       ...
       function index()
		...
		//$this->form_validation->set_rules('type', 'Username');
		$this->form_validation->set_rules('type1', 'required');
		...
		//$this->form_validation->set_rules('type6', 'required');
